...
PRINTCONFIRM_URL="https://backend.weh.rwth-aachen.de/PrintConfirm.php"
...
inotifywait -m -e modify "$LOGFILE" | while read file event; do
    ...
    for JOB_ID in $JOB_IDS; do
        echo "✅ Job abgeschlossen: $JOB_ID - Starte PrintConfirm.php"
        curl "$PRINTCONFIRM_URL?cups_id=$JOB_ID" &
    done
done
