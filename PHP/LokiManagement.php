...
if (auth($conn) && ($_SESSION['NetzAG'] || $_SESSION['Vorstand'] || $_SESSION["aktiv"])) {
  ...
  #echo '<pre style="color: white;">';print_r($_POST);echo '</pre>';
    ...
    if (isset($_POST["reload"]) && $_POST["reload"] == 1) {
        if (isset($_POST["upload_infopic"])) {
            ...
            if (!isset($_FILES['file']) || $_FILES['file']['error'] != UPLOAD_ERR_OK) {
                ...
                $insert_sql = "INSERT INTO infopics (uid, tstamp, aktiv, beschreibung, pfad) VALUES (?,?,?,?,?)";
                $insert_var = array($uid, $zeit, $aktiv, $beschreibung, $file_path);
    ...
    $sql = "SELECT id, beschreibung, aktiv, pfad, changelog FROM infopics WHERE aktiv > -1";
