...
function d2_collect_dashboard_data(mysqli $conn): array
{
    ...
    if ($endtime) {
        ...
        $certState = ((int)$alert > 0) ? 'bad' : 'good';
        $certCompact = true;
    } else {
...
<head>
    ...
    <style>
        ...
        .d2-left { min-width: 0; min-height: 0; height: 100%; box-sizing: border-box; overflow: hidden; }
        .d2-left-shell {
            height: 100%;
            min-height: 0;
            display: grid;
        ...
        .d2-panel-title { font-size: clamp(18px, 1.3vw, 23px); font-weight: 950; color: #dfffdc; }
        ...
        .d2-terminal-title { font-weight: 950; letter-spacing: .02em; }
        ...
        .d2-terminal-send { border: 0; border-radius: 12px; padding: 9px 12px; background: #11a50d; color: #fff; font-weight: 950; cursor: pointer; }
        ...
        .d2-modal-title { font-size: 22px; font-weight: 900; }
        .d2-modal-close { border: 0; background: #fff; color: #111; width: 34px; height: 34px; border-radius: 999px; cursor: pointer; font-weight: 900; }
        ...
        .d2-input-bad { color: #ff6b6b !important; font-weight: 900; }
        ...
        .d2-green { color: #35d235; font-weight: 900; }
        .d2-red { color: #ff5f5f; font-weight: 900; }
        .d2-warn { color: #E49B0F; font-weight: 900; }
        .d2-submit { display: block; margin: 18px auto 0; border: 0; border-radius: 16px; padding: 12px 20px; background: #fff; color: #111; font-weight: 900; cursor: pointer; min-width: 180px; }
        ...
        <main class="d2-left">
            <section class="d2-left-shell">
                ...
                <section class="d2-section d2-script-section">
                    <div class="d2-panel-head">
                        <div class="d2-panel-title">Skripte</div>
                    </div>
  ...
  grid.innerHTML = Object.values(queues).map(queue => {
    ...
    return `
      <article class="d2-queue-card${items.length ? " d2-has-items" : ""}">
        ...
        <div class="d2-item-list">${buttons}</div>
      </article>`;
  }).join("");
}
...
function d2RenderScripts() {
  ...
  grid.innerHTML = Object.values(crons)
    .filter(script => script.key !== "pskabgleich" && !script.hidden)
    .map(script => `
